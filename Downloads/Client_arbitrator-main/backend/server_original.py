...
async def register(user_data: UserCreate):
    ...
    user_dict['email_verified'] = is_arbitrator  # Arbitrator doesn't need verification
    
    await db.users.insert_one(user_dict)
    
    # If arbitrator, auto-verify and login
    if is_arbitrator:
...
async def login(credentials: UserLogin):
    ...
    # Check email verification (except for arbitrator)
    if user['email'] != ARBITRATOR_EMAIL and not user.get('email_verified', False):
...
async def get_disputes(current_user: dict = Depends(get_current_user)):
    if current_user['role'] != 'arbitrator':
